...
class ShadowRemovalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        shadow_img = Image.open(os.path.join(datatype_dir, 'shadow', '{:0>6d}.png'.format(img_id)))
        mask_img = Image.open(os.path.join(datatype_dir, 'mask', '{:0>6d}.png'.format(img_id)))
        shadow_free_img = Image.open(os.path.join(datatype_dir, 'non_shadow', '{:0>6d}.png'.format(img_id)))
        ...
        if self.transform:
            shadow_img, mask_img, shadow_free_img = self.transform(shadow_img, shadow_free_img, mask_img)
...
# TODO: add augmentation
def get_composed_augmentation(aug_dict=None):
    augmentations = []
    if not aug_dict:
        aug_dict = {
