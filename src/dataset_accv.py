...
class ShadowRemovalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.data_type == 'training' or self.data_type == 'validation':
            ...
            accv_dir = os.path.join(self.path.data_dir, 'result', 'ACCV2016', 'train')
        elif self.data_type == 'testing':
            datatype_dir = self.path.test_dir
            accv_dir = os.path.join(self.path.data_dir, 'result', 'ACCV2016', 'test')
        else: 
        ...
        if self.transform:
            shadow_img, shadow_free_img, mask_img, accv_img = self.transform(shadow_img, shadow_free_img, mask_img, accv_img)
        ...
        accv_img = tfs(accv_img)
        #print(shadow_img.size())
        return shadow_img, shadow_free_img, mask_img, accv_img
...
class Compose(object):
    ...
    def __call__(self, shadow_img, shadow_free_img, mask_img, accv_img):
        
        for aug in self.augmentations:
            shadow_img, shadow_free_img, mask_img, accv_img = aug(shadow_img, shadow_free_img, mask_img, accv_img)
...
class Resize(object):
    ...
    def __call__(self, shadow_img, shadow_free_img, mask_img, accv_img):
        # print('resize')
        assert shadow_img.shape[0] == shadow_free_img.shape[0] and shadow_img.shape[1] == shadow_free_img.shape[1]
        assert shadow_img.shape[0] == mask_img.shape[0] and shadow_img.shape[1] == mask_img.shape[1]
        assert shadow_img.shape[0] == accv_img.shape[0] and shadow_img.shape[1] == accv_img.shape[1]
        ...
        accv_img = cv2.resize(accv_img, (self.width, self.height), cv2.INTER_LINEAR)
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, shadow_img, shadow_free_img, mask_img, accv_img):
        ...
        if random.random() >= self.prob:
            ...
            accv_img = np.fliplr(accv_img).copy()
...
class RandomVerticalFlip(object):
    ...
    def __call__(self, shadow_img, shadow_free_img, mask_img, accv_img):
        ...
        if random.random() >= self.prob:
            ...
            accv_img = np.flipud(accv_img).copy()
...
class RandomRotation(object):
    ...
    def __call__(self, shadow_img, shadow_free_img, mask_img, accv_img):
        ...
        accv_img = imutils.rotate(accv_img, rotate_degree)
...
class RandomCrop(object):
    ...
    def __call__(self, shadow_img, shadow_free_img, mask_img, accv_img):
        ...
        accv_img = accv_img[y : y + self.height, x : x + self.width, :]
