...
class STCGAN_CONCAT():
    ...
    def forward(self, pair):
        ...
        pair['N_fake'] = self.G2(pair['SAM_fake'])
        pair['SAMN_real'] = torch.cat((pair['SAM_real'], pair['N_real']), 1)
        pair['SAMN_fake'] = torch.cat((pair['SAM_fake'], pair['N_fake']), 1)
    ...
    def calculate_D_loss(self, pair):
        ...
        D1_real = self.D1(pair['SAM_real'])
        D1_fake = self.D1(pair['SAM_fake'].detach())
        # print('D1_real shape: {}\n-------------------'.format(D1_real.size()))
        ...
        D2_real = self.D2(pair['SAMN_real'])
        D2_fake = self.D2(pair['SAMN_fake'].detach())


        D2_real_loss = self.gan_loss(D2_real, True)
        D2_fake_loss = self.gan_loss(D2_fake, False)
    ...
    def calculate_G_loss(self, pair):
        ...
        D1_fake = self.D1(pair['SAM_fake'])
        D2_fake = self.D2(pair['SAMN_fake'])
